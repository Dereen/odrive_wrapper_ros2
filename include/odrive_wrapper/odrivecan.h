...
#ifndef ODRIVE_WRAPPER_CAN_H
#define ODRIVE_WRAPPER_CAN_H
 ...
 */
    ...
    public:
        ...
        OdriveCan(uint8_t axes_num, struct OdriveCanParams param, std::ostream *const out, std::ostream *const err) : axes_num(axes_num), output_stream(out), error_stream(err) {
    ...
    std::ostream& operator<<(std::ostream &out, const OdriveCan &odrive); // Forward stream function
}

#endif // ODRIVE_WRAPPER_CAN_H